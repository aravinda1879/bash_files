...
proc join_sel { sel_wrap } { 
	package require pbctools
	pbc join seg -sel "$sel_wrap" -all 
        #pbc wrap  -center com -centersel "$sel_wrap" -all 
}

proc logana {mol in_file_mass in_file_log} {
	set infile_m [open $in_file_mass r]
	set num_mass [ lindex [gets $infile_m] 1]
	close $infile_m

}
# miniimage calculate between 2 selections
proc minimage {mol ini_frame  sel_1 sel_2 out_file cutoff} {
	...
	for { set frame $ini_frame } { $frame < $num_frames } { incr frame } {
		...
		set atomlist [ lsort -unique [ $sel2 get residue  ] ]
		puts $outfile "[expr $frame - $ini_frame]|[llength $atomlist]"
	}
	close $outfile
}
#minimage_all one selection and what comes around it
proc minimage_all {mol ini_frame  sel_1 out_file { cutoff "12" }} {
	set outfile [open $out_file w]
	set num_frames [molinfo $mol get numframes]
	set num_steps [expr $num_frames - $ini_frame ]
	puts $outfile "number_of_steps $num_steps"
	set sel1 [ atomselect top "$sel_1" ]
	set reslist [ lsort -unique [ $sel1 get residue  ] ]
        if  { $ini_frame < 0 } {
                set ini_frame 0
        }
	for { set frame $ini_frame } { $frame < $num_frames } { incr frame } {
		set tot_contact($frame) 0
                animate goto $frame
		foreach residue ${reslist} {
			set sel2 [ atomselect top "(pbwithin ${cutoff} of residue $residue) and not (within ${cutoff} of residue $residue) and not ( water or ion)" ]
			set temp_count [llength  [ lsort -unique [ $sel2 get residue  ] ]]
			set tot_contact($frame) [expr $temp_count + $tot_contact($frame) ]
		}
		puts $outfile "[expr $frame - $ini_frame]|$tot_contact($frame)"
	}
	close $outfile
}
...
proc images { ini_frame num_pdbs sel_1 { psf_name "caver.psf" } } {
	...
	for { set frame $ini_frame } { $frame < $num_frames } { incr frame $stp_sz  } {
		...
		animate write pdb [expr $frame - $ini_frame].pdb beg $frame end $frame skip 0 waitfor 1 sel $sel1 
	}
...
proc msd_com { mol sel_1 ini_frame end_frame f_name} {
	set sel1 [atomselect $mol "$sel_1"]
	$sel1 frame 0
	set r0 [ measure center $sel1 weight mass ]
	set num_steps [molinfo top get numframes]
	set outfile [open "${f_name}_com.dat" w]
	for {set i 0} {$i < $num_steps} {incr i} {
		$sel1 frame $i
		set rt [ measure center $sel1 weight mass ]
		set d_rt [veclength2 [vecsub $rt $r0]]
		puts $outfile "$i $d_rt"
	}
	close $outfile
	puts "time vs MSD is done"
...
# add the water dipole as a sub function with a if conditiion
proc contact_bigdcd { frame } {
	#need to convert all the following contact analysis to residue number from resid
	global sel1 sel2 cutoff outfile cen_sel fnum wat_dip_ana wat_dip_outfile
	set fnum $frame
	package require pbctools
	pbc wrap -center com -centersel "$cen_sel" -now -compound residue
	set atomlist [ measure contacts $cutoff $sel1 $sel2 ]
	puts $outfile "[expr $frame ]|[lindex $atomlist 0]|[lindex $atomlist 1]"
	if { $wat_dip_ana=="yes" } {
		set dp_lst [ water_dipole_bigdcd $atomlist ]
		puts $wat_dip_outfile "[expr $frame ]|$dp_lst"
	}

}
#assuming sel2 is water
proc water_dipole_bigdcd { atomlist  } {
	set dp_lst []
	foreach i [lindex $atomlist 0] j [lindex $atomlist 1] {
		set sel_wat     [ atomselect top "index $j" ]
		set sel_not_wat [ atomselect top "index $i" ]
		set wat     [ measure center $sel_wat     ]
		set not_wat [ measure center $sel_not_wat ]
		set dis_vec [ vecnorm [vecsub $not_wat $wat ]]
		set sel_dp  [ atomselect top "residue [$sel_wat get residue]" ]
		set dp [vecnorm [measure dipole $sel_dp  ]]
		set angle [expr acos([ vecdot  $dis_vec $dp]) ]
		lappend dp_lst $angle
		$sel_wat delete
		$sel_not_wat delete
		$sel_dp delete
	}
...
proc pol_prot_freq { mol in_file sel_1 sel_2 ou_file } {
	set infile [open $in_file r]
	set num_steps [ lindex [gets $infile] 1]
	set tmp_res_num [ lindex [gets $infile] 1]
	set resnum_lst1  [lsort -unique  [[atomselect top "$sel_1"] get {residue} ]]
	set resnum_lst2  [lsort -unique  [[atomselect top "$sel_2"] get {residue} ]]
	#list 1 will be the polymer and list 2 will be the protein
	set res_max1 [lindex [lsort -integer -decreasing $resnum_lst1] 0]
	set res_min1 [lindex [lsort -integer -increasing $resnum_lst1] 0]
	set res_max2 [lindex [lsort -integer -decreasing $resnum_lst2] 0]
	set res_min2 [lindex [lsort -integer -increasing $resnum_lst2] 0]
	puts "$res_max1 $res_min1 and $res_max2 $res_min2"
	#defining array for each matrix
	for {set i $res_min1} {$i <= $res_max1 } {incr i} {
		for {set j $res_min2} {$j <= $res_max2 } {incr j} {
			set residue_freq($i,$j) 0
		}
	}
	parray residue_freq
	puts "reading the input file"
	while {[gets $infile line] != -1} {
		set t_con [split $line "|" ]
		set pol_lst [split [lindex $t_con 1] " " ]
		set pro_lst [split [lindex $t_con 2] " " ]
		set pair_len [llength $pol_lst]
		set pair_len2 [llength $pro_lst]
		if { $pair_len != 0 } {
			set tmp_lst [list ]
			#puts "line [lindex $t_con 0]"
			foreach t1 $pol_lst t2 $pro_lst {
				set del1 [atomselect top "index $t1"]
				set del2 [atomselect top "index $t2"]
				set pol_residue [ $del1 get residue ]
				set pro_residue [ $del2 get residue ]
				lappend tmp_lst "$pol_residue $pro_residue"
				$del1 delete
				$del2 delete
			}
			set tmp_lst_2 [lsort -unique $tmp_lst]
			foreach i $tmp_lst_2 {
				#puts "here [lindex $i 0] [lindex $i 1]"
				#puts ""
				set residue_freq([lindex $i 1],[lindex $i 0]) [expr ($residue_freq([lindex $i 1],[lindex $i 0]) + 1)]
			}
		}
	}
	set outfile [open $ou_file w]
	puts $outfile "protein_min $res_min1 protein_max $res_max1 "
	puts $outfile "polymer_min $res_min2 polymer_max $res_max2 "
	for {set i $res_min1} {$i <= $res_max1 } {incr i} {
		for {set j $res_min2} {$j <= $res_max2 } {incr j} {
			puts $outfile "$i $j $residue_freq($i,$j)"
		}
	}
	close $outfile
	puts "Done"	
...
#similar to text processing as contact_res_freq procedure
#this procedure based on residue number (in old we used resid)    2D exp
proc contact_pol_freq { mol in_file sel_1 ou_file } {
	set infile [open $in_file r]
	set num_steps [ lindex [gets $infile] 1]
	set tmp_res_num [ lindex [gets $infile] 1]
	set count 0
	set resnum_lst  [lsort -unique -index 0 [[atomselect top "$sel_1"] get {residue resid segname} ] ]
	set resnum_lst2  [lsort -unique -index 0 [[atomselect top "$sel_1"] get {residue} ]]
	foreach i $resnum_lst2 {set res_freq($i) 0 }
	foreach i $resnum_lst2 {set res_time($i) 0 }
	foreach i $resnum_lst2 {set res_time_norm($i) 0 }
	puts "reading the input file"
	while {[gets $infile line] != -1} {
		set t_con [split $line "|" ]
		set pol_lst [split [lindex $t_con 1] " " ]
		set pair_len [llength $pol_lst]
		if { $pair_len != 0 } {
			set del1 [atomselect top "index $pol_lst"]
			set pol_residue [lsort -unique [ $del1 get residue ] ]
			foreach t2 $pol_residue {
	...
	set outfile [open $ou_file w]
	puts $outfile "# residue resid segname total_time"
	foreach i $resnum_lst {
		puts $outfile "[lindex $i 0] [lindex $i 1] [lindex $i 2] $res_time([lindex $i 0])"
	}
	close $outfile
...
#contact timeline  for each residue ou_file = contact residue vs time -- based on residue number. once this is finalized delete the previous version which was based on resID
proc residue_contact_timeline {mol in_file out_file_1 out_file_2 } {
        set in_file [open $in_file r]
        set num_steps [ lindex [gets $in_file] 1]
        set lst_contact {}
	...
	set lst_unique_reduce_contact {}
        ...
        while {[gets $in_file line] != -1} {
            ...
            if {$tmp_lst_reduce_contact  != {} } {
                set del_residue [atomselect top "index $tmp_lst_reduce_contact"] 
                set tmp_lst_reduce_contact_residue [$del_residue get residue]
                set unique_tmp_lst_reduce_contact_residue [ lsort -unique $tmp_lst_reduce_contact_residue ]
		$del_residue delete
                lappend lst_reduce_contact $tmp_lst_reduce_contact_residue 
		lappend lst_unique_reduce_contact $unique_tmp_lst_reduce_contact_residue 
                } else {
...
#contact timeline  for each residue ou_file = contact residue vs time -- > based on resID. delete this once previous one is done
proc resid_contact_timeline {mol in_file out_file_1 out_file_2 } {
        set in_file [open $in_file r]
        set num_steps [ lindex [gets $in_file] 1]
        set lst_contact {}
        set lst_reduce_contact {}
