...
proc patch_resinfo {mol in_file in_file_2 out_file   {frame_load_time "10"}  {cutoff "25"}} {
	...
	for { set res_id 0 } { $res_id <= $rlast } {incr res_id} {
		set line [split $res_con_list " " ]
		if { [lindex $line 1] > [expr double($cutoff)*1000/double($frame_load_time)] } {
			set del1 [atomselect $mol "resid [lindex $line 0 ] "]
			set resinfo [$del0 get {residue segname resid resname}]
			lappend filtered_resinfo $resinfo 
			lappend filtered_time [list [lindex $line 1 ], [lindex $line 2 ]]
			$del1 delete 
		}
	}
	set outfile1 [open $out_file w]
	puts $outfile "#resid residue segname resname time(ns) normalized_time(persimulation)"
	for {set i 0} { $i < $rlast } {incr i} {puts $outfile "[lindex $filtered_resinfo $i 2] [lindex $filtered_resinfo $i 0] [lindex $filtered_resinfo $i 1] [lindex $filtered_resinfo $i 3] [lindex $filtered_time $i 0] [lindex $filtered_time $i 1]"}
	puts "Done printing residue information and time to the file"
}
# measure angle between three selections
proc angle_3_com { mol sel_1 sel_2 sel_3 dum_sel_1 dum_sel_2 dum_sel_3 f_name { sel_wrap "wrap_true"  } } {
...
proc sasa_cal {mol f_name sel restrict { wrap "yes" } { center_sel "protein"   } } {
...
proc print_rmsd_through_time {mol f_name sel_1 sel_2} {
        set reference [atomselect $mol "$sel_1" frame 0]
        set compare [atomselect $mol "$sel_1"]
        set measure_rmsd_ref [atomselect $mol "$sel_2" frame 0]
        set measure_rmsd_compare [atomselect $mol "$sel_2"]
        set num_steps [molinfo $mol get numframes]
        set outfile [open $f_name w]
        for {set frame 0} {$frame < $num_steps} {incr frame} {
                $compare frame $frame
                $measure_rmsd_compare frame $frame
                set trans_mat [measure fit $compare $reference]
                $measure_rmsd_compare move $trans_mat
                set rmsd [measure rmsd $measure_rmsd_compare $measure_rmsd_ref]
...
proc distance { mol a_1 a_2 f_name} {
	...
	set list_distance [list ]
		...
		lappend list_distance [measure bond [list $a_1 $a_2] frame $frame]
...
proc radial { mol sel1 sel2 r_max ou_file fst lst { wrap "yes"} } {
	if {$wrap=="1"} {
		package require pbctools
		pbc wrap -center com -centersel "protein and not hydrogen" -all -compound residue
	}
...
proc contact {mol sel_1 sel_2 cutoff f_name { wrap "yes" }  { wrap_center "protein" } } {
        set num_steps [molinfo $mol get numframes]
	...
	set con_lst {}
        ...
        for { set frame 0 } { $frame < $num_steps } { incr frame } {
                ...
                puts $outfile "$frame|[lindex $atomlist 0]|[lindex $atomlist 1]"
                # $sel1 delete
                #  $sel2 delete
        }
        close $outfile
        ...
        for { set t_stp 0 } { $t_stp < $num_steps } {incr t_stp} {
		    ...
		    set pol_lst [split [lindex $t_con 1] " " ]
                ...
                if { $pair_len != 0 } {
                        ...
                        for { set k 0 } { $k < $pair_len } { incr k } {
                                set pro_1 [lindex $pro_lst $k]
...
proc res_contact_timeline {mol in_file ou_file} {
        set in_file [open $in_file r]
        set num_steps [ lindex [gets $in_file] 1]
        set lst_contact {}
        ...
        set outfile [open $ou_file w]
        for {set i 0} { $i < $num_steps } {incr i} {puts $outfile "$i|[lindex $lst_reduce_contact $i]"}
