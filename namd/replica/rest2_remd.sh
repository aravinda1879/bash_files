...
binCoordinates     $Bincoordinates;     # coordinates from last run (binary)
extendedSystem     $extendedsystem;     # cell dimensions from last run (binary)
temperature \$TEMP
}

outputEnergies [expr \$steps_per_run ]    #dividing by ten so intermidiate outputs will be on log
