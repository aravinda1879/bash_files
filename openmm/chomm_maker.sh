...
#========================== Initialize simulation system
 ...
 for pf in paramfile:
     dprint("Reading parameters from file '", pf,"'");
 params=app.CharmmParameterSet(*paramfile, permissive=False); # running without atom typing (via mass entries in the topology) often leads to ERRORS !
#========================================================
 if (pbc):
...
#
  ...
  force.addGlobalParameter("k",500.0*u.kilocalorie/u.mole/u.angstrom/u.angstrom)
  force.addPerParticleParameter("x0");
  force.addPerParticleParameter("y0");
  force.addPerParticleParameter("z0");
  force.addGlobalParameter("s", constraintscaling);
# read per atom restraints :
  pdb=app.PDBFile(consfile);
  atoms = list(pdb.topology.atoms())
  icons=0;
  for iatom, atom_crd in enumerate(pdb.positions):
   if atoms[iatom].name in ('CA', 'C', 'N'):
    force.addParticle(iatom, atom_crd)
    icons += 1
